	...
	"escapeHtmlAttribute" : function(data) {
		...
		for (var i = 0; i < data.length; i++) {
			if (helmet.safeInAttributes.indexOf(data.charAt(i)) > -1) {
				...
				if ((ccode & 0xF800) == 0xD800 && !surrogate) {
					surrogate = ccode;
					continue;
				}
				var padLength = ccode < 0x100 ? 2 : 4;
				if (surrogate) {
					ccode = ((surrogate & 0x3FF) << 10) + (ccode & 0x3FF) + 0x10000
					result.push("&#x" + ccode.toString(16) + ";");
					surrogate = null;
				} else {
					result.push("&#x" + helmet.zeropad(ccode.toString(16), padLength) + ";");
				}					
			}
		}
		return result.join("");
